...
Partial Class news
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Else
            LitTitle.Text = "鑫梅企業股份有限公司>>最新消息"
            show1.Visible = True
            show2.Visible = False
        End If
        If Request("NID") = "" Then
            LitMeta.Text = "<META NAME=""KEYWORDS"" CONTENT=""鑫梅企業股份有限公司,最新消息"">" & vbCrLf
        End If
    End Sub
#Region "顯示清單"
    Private Sub ShowTable()
        Dim ds As DataSet
        ds = ClassDB.RunReturnDataSet("select * from New where Isonline=true and SiteID=1 order by PostDate Desc", "News")
        ...
        '這裡是分頁的
        If list = "" Then
            LitHtml.Text = "<tr><td colspan=3 align=center><font color=red>目前暫無最新消息文章！</font></td></tr>"
            LitPage.Text = ""
        Else
...
#Region "顯示資料"
    Private Sub ShowData(ByVal NID As Integer)
        Dim dr As OleDbDataReader = ClassDB.GetDataReader("select * from New where NID=" & NID & " and SiteID=1")
        Dim FileName As String
        If dr.Read Then
            LitMeta.Text = "<META NAME=""KEYWORDS"" CONTENT=""鑫梅企業股份有限公司,最新消息," & dr("Subject").ToString & """>" & vbCrLf
            LitTitle.Text = "鑫梅企業股份有限公司>>最新消息>>" & dr("Subject").ToString
