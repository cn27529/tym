...
Partial Class English_CommonWeb_Product_left
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        Dim i As Integer
        Sql = "select PublicID,Subject from Product where IsDel=false and Isonline=true and SiteID=2 and ParentPublicID=0 and Lvl=1 order by OrderData"
    ...
    Private Sub ChildMenu2(ByVal PublicID As String)
        Try
            Dim dr As OleDbDataReader = ClassDB.GetDataReader("select PublicID,Subject,ProductName,Kind from Product where IsDel=false and Isonline=true and SiteID=2 and ParentPublicID=" & PublicID & " order by OrderData")
            While dr.Read
    ...
    Private Sub ChildMenu3(ByVal PublicID As String)
        Try
            Dim dr As OleDbDataReader = ClassDB.GetDataReader("select PublicID,Subject,ProductName,Kind from Product where IsDel=false and SiteID=2 and ParentPublicID=" & PublicID & " order by OrderData")
        Catch ex As Exception
            ' WriteErrLog(ex, Me.Page)
        Finally
        End Try
